# UM Monopoly — Architecture & Coding Design
...
**Alternative:** Godot 4 (GDScript) — lighter, fully open-source. Pick if team has more Godot experience.
...
Doubles → re-roll (max 3 in a row → jail).
...
These are designer-editable in Unity Inspector — no code changes needed to balance.
...
│   └── EndGame.unity
├── Scripts/
│   ├── Core/          (GameManager, TurnController, GameState)
│   ├── Entities/      (Player, Tile + subclasses, Card)
│   ├── Systems/       (Bank, CardDeck, DiceRoller, SaveSystem)
│   ├── UI/            (HUD, popups, menus)
│   └── Data/          (ScriptableObjects)
├── Data/
│   ├── Tiles/         (40 TileData assets)
│   └── Cards/         (Card data assets)
└── Resources/
```
...
| Art bottleneck | Med | Med | Use placeholder art until Week 6; finalize art Week 10–11 |
...
1. **AI opponent?** — defer decision until Week 8 based on velocity
2. **Animation framework?** — Unity Animator vs DOTween (recommend DOTween for code-driven tweens)
3. **Localization?** — out of scope unless trivial; English with BM flavor words inline
